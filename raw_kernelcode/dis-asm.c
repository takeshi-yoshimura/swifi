/*
 * dis-asm.c -- x86 disassembler for linux kernel
 * This file is originally part of KDB and modified by Takeshi Yoshimura in 2013.
 *
 *
...
SYSCALL_DEFINE2(disasm, unsigned long, address, char __user *, str){
    ...
    if (ret > 0) {
        if (copy_to_user(str, str2, 256)) {
            printk(KERN_WARNING "copy_to_user failed at sys_disasm\n");
            ret = 0;
        }
    }
    kfree(str2);
