...
SYSCALL_DEFINE4(swifi2, const char __user *, name, int, cmd, unsigned long, seed, struct swifi_result2 __user *, result)
{
    ...
    printk(KERN_INFO "------------------sys_swifi2-------------------\n");
    printk(KERN_INFO "cmd=%2d seed=%lu\n", cmd, seed);
    printk(KERN_INFO "addr: %lx (%s)\n", 
	(unsigned long)result->target.low, result->str);
    ...
    printk(KERN_INFO "addr2: %lx (%s)\n", 
	(unsigned long)result->target.high, result->str2);
    printk(KERN_INFO "change %016lx %016lx to %016lx %016lx\n",
	(unsigned long)result->old_content2.low, 
	(unsigned long)result->old_content2.high,
	(unsigned long)result->new_content2.low, 
	(unsigned long)result->new_content2.high);
    printk(KERN_INFO "old instruction2: %s\n", result->old_instr2);
    printk(KERN_INFO "new instruction2: %s\n", result->new_instr2);
    printk(KERN_INFO "------------------sys_swifi2-------------------\n");
